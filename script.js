...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("database.json")
    ...
    .then((data) => {
      if (Array.isArray(data) && data.length > 0) {
        passedDatabase = data;
        console.log(
          "Database kelulusan ORIMA 2026 berhasil dimuat dari database.json:",
          data
        );
      }
    })
    .catch((err) => {
      console.warn(
        "Menggunakan database fallback kelulusan (file:// mode):",
        err.message
      );
    });
    ...
    fetch("nomor_pendaftaran_nama_ORIMA_2026.json")
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          lookupDatabase = data;
          console.log(
            "Dataset lookup nomor pendaftaran ORIMA 2026 dimuat:",
            data.length,
            "peserta"
          );
        }
      ...
      .catch((err) => {
        console.warn(
          "Gagal memuat dataset lookup nomor pendaftaran:",
          err.message
        );
      });
  }
  ...
  const btnCloseLookupFooter = document.getElementById(
    "btn-close-lookup-footer"
  );
  ...
  btnEnter.addEventListener("click", () => {
    ...
    if (bgMusic) {
      ...
      bgMusic
        .play()
        .catch((err) => console.warn("Musik tidak bisa diplay:", err));
    }
    ...
    if (matches.length === 0) {
      ...
      lookupResultsList.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Nama <strong>"${escapeHtml(
                      query
                    )}"</strong> tidak ditemukan dalam daftar peserta.</p>
                ...
                <div class="lookup-item">
                    <div class="lookup-info">
                        ...
                        <span class="lookup-nomor-badge">
                            <i class="fa-solid fa-id-card"></i> ${escapeHtml(
                              item.nomor_pendaftaran
                            )}
                        </span>
                    </div>
                    <button type="button" class="btn-select-nomor" data-nomor="${escapeHtml(
                      item.nomor_pendaftaran
  ...
  function highlightMatch(text, query) {
    ...
    return safeText.replace(
      regex,
      '<mark style="background: rgba(251, 191, 36, 0.35); color: #fbbf24; border-radius: 2px; padding: 0 2px;">$1</mark>'
    );
  }
  ...
  function checkGraduationStatus(inputNomor, inputNama) {
    ...
    const matchedCandidate = passedDatabase.find((item) => {
      const matchNomor =
        item.nomor_pendaftaran.toString().toLowerCase() === cleanNomor;
  ...
  function renderLulus(candidate) {
    ...
    resPesan.textContent =
      "Selamat atas kelulusan Anda dalam Seleksi Open Recruitment ORIMA 2026 UKM PRIMA UNRAM (Kabinet UNI7Y)! Perjuangan dan dedikasi Anda telah membuahkan hasil. Teruslah mengeksplorasi cipta karya, leadership, dan inovasi bersama UKM PRIMA.";
  ...
  function renderTidakLulus(nomor, nama) {
    ...
    resPesan.textContent =
      "Mohon maaf, data pendaftaran Anda tidak tercantum dalam daftar kelulusan Open Recruitment ORIMA 2026 UKM PRIMA UNRAM. Terima kasih atas partisipasi dan semangat luar biasa yang telah Anda tunjukkan. Jangan berkecil hati, kegagalan adalah awal dari keberhasilan. Tetap semangat dan terus berkarya! | see u in next ORIMA 2027 !!";
